...
arrowSkin: '',
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuDesatB'));
				bg.scrollFactor.set();
		bg.color = 0xFF222222;
		add(bg);
		...
		eventIcon.setGraphicSize(45, 45);
		...
		leftIcon.setPosition(GRID_SIZE + 28, -100);
		rightIcon.setPosition(GRID_SIZE * 5.55, -100);
		...
		//FlxG.save.bind('ffm', CoolUtil.getSavePath());
		...
		camPos = new FlxObject(0, 0, 1, 310);
		...
		UI_box.resize(350, 475);
		UI_box.x = 890 + GRID_SIZE / 2;
		...
		with Snapping
		\nLeft Bracket / Right Bracket - Change Song Playback
		Rate (SHIFT to go Faster)
		\nALT + Left Bracket / Right Bracket - Reset Song 
		Playback Rate
		...
		for (i in 0...tipTextArray.length) {
			var tipText:FlxText = new FlxText(0, -110 + UI_box.height + 8, 0, tipTextArray[i], 16);
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("Brose.ttf"), 8, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
...
noteTypeMap.set(noteTypeList[key], key);
		...
		directories.push(Paths.getPreloadPath('lua/custom_notetypes/'));
		for (i in 0...directories.length) {
			var directory:String =  directories[i];
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
	...
	function addEventsUI():Void
	{
		...
		var directories:Array<String> = [Paths.getPreloadPath('lua/custom_events/')];
		for (i in 0...directories.length) {
			var directory:String =  directories[i];
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
	...
	function addChartingUI() {
		...
		if (FlxG.save.data.chart_vortex == null) FlxG.save.data.chart_vortex = true;
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("Brose.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -26;
				daText.yAdd = 11;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("Brose.ttf"), 8, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
